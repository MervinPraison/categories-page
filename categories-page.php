<?php
/*
Plugin Name: Categories Page
Plugin URI: https://wordpress.org/plugins/categories-page/
Description: Displays a table listing of all Categories registered on your website. Shortcode.
Version: 1.3
Author: Mervin Praison
Author URI: https://mer.vin
License: GPLv2 or later
License URI: https://www.gnu.org/licenses/gpl-2.0.html
Text Domain: categories-page
Domain Path: /lang
*/
...
class CategoriesPage {
  ...
  public function __construct() {
    ...
    add_shortcode('get_categories', array($this, 'shortcode'));
  ...
  public function load_textdomain() {
    load_plugin_textdomain('categories-page', false, basename(dirname(__FILE__)) . '/lang/');
  }
  ...
  public function enqueue_scripts() {
    ...
    wp_register_style('categories-page', plugins_url("/css/categories-page$suffix.css", __FILE__), array(), self::version);
    wp_enqueue_style('categories-page');

    wp_register_script('webtoolkit.sortabletable', plugins_url("/js/webtoolkit.sortabletable$suffix.js", __FILE__), array(), self::version, true);
  ...
  public function admin_enqueue_scripts($hook) {
    if($hook != 'widgets.php')
      return;
   	$suffix = SCRIPT_DEBUG ? '' : '.min';
    wp_enqueue_style('categories-page-admin', plugins_url("/css/admin$suffix.css", __FILE__), array(), self::version);
    wp_enqueue_script('categories-page-admin', plugins_url("/js/admin$suffix.js", __FILE__), array('jquery', 'wplink'), self::version, true);
    wp_enqueue_style('editor-buttons');
    require_once(ABSPATH . WPINC . '/class-wp-editor.php');
    add_action('admin_footer', array($this, 'wp_link_dialog_footer'));
  }

  public function wp_link_dialog_footer() {
  ...
  public function shortcode($atts) {
    extract( shortcode_atts(array(
  	...
  	$tag_caption = __('Tag', 'categories-page');
  	$count_caption = __('Count', 'categories-page');
    ...
    if ($tags) {
    	...
    	$view_link = __("View all posts in %s", 'categories-page');
    	if (!isset($atts['pagesize']) || !$atts['pagesize'] || is_nan($atts['pagesize']) || $atts['pagesize'] < 1) {
      	$start = 0;
      	$end = count($tags);
      } else {
      	$start = 0;
      if (isset($_SERVER['REQUEST_URI']) && preg_match("#^(.+)/page/(\d+)#i", esc_url_raw(wp_unslash($_SERVER['REQUEST_URI'])), $match)) {
      ...
      for ($i = $start; $i < $end; $i++) {
        $tag = $tags[$i];
      	$ret .= sprintf('<tr>
      	         ...
      	         </tr>', esc_html($tag->name), $tag->count, esc_url(get_category_link($tag->term_id)), esc_attr(sprintf($view_link, $tag->name)));
      }
    }
  	...
  	$ret .= sprintf('  </tbody>
  	         ...
  	         </table>', esc_html($tag_caption), esc_html($count_caption));
  	
  	if ($start > 0 || $end < count($tags)) { // pagination bottom
  	  $page = ceil($end / $atts['pagesize']);
  	  $page_count = ceil(count($tags) / $atts['pagesize']);
  	  $ret .= '<div class="categories-page-pagination">';
  	  /* translators: 1: current page number, 2: total number of pages */
  	  $label = sprintf(__('Page %1$s of %2$s', 'categories-page'), $page, $page_count);
  	  $ret .= "<span>$label</span>";
  	  $pstart = $page <= 3 ? 1 : $page - 2;
  	  $pend   = $page + 3 >= $page_count ? $page_count : $page + 2;
  	  if (empty($match)) {
  	    $base_uri = isset($_SERVER['REQUEST_URI']) ? esc_url_raw(wp_unslash($_SERVER['REQUEST_URI'])) : '';
  	    if (substr($base_uri, -1) == "/")
  	      $base_uri = substr($base_uri, 0, strlen($base_uri) - 1);
  	    $match = array(1 => $base_uri);
  	  }
  	  if ($pstart > 1)
  	    $ret .= sprintf('<a href="%1$s" class="first-page">1</a> … ', esc_url($match[1]));
  	  for ($i = $pstart; $i <= $pend; $i++)
  	    if ($page == $i)
  	      $ret .= sprintf('<span class="current-page">%1$d</span>', $i);
  	    else
  	      $ret .= sprintf('<a href="%1$s/page/%2$d">%2$d</a>', esc_url($match[1]), $i);
  	  if ($pend < $page_count)
  	    $ret .= sprintf(' … <a href="%1$s/page/%2$d" class="last-page">%2$d</a>', esc_url($match[1]), $page_count);
  	  $ret .= '</div>';
  	}

    wp_enqueue_script('webtoolkit.sortabletable');
    if (!CategoriesPage::$init_table) { // init table sorting in page footer, but only once
      add_action('wp_footer', array($this, 'footer'));
      CategoriesPage::$init_table = true;
    }
          ...
          foreach ($widget_tag_cloud_option as &$tag_cloud) {
            if (is_array($tag_cloud) && isset($tag_cloud['taxonomy']) && $tag_cloud['taxonomy'] == 'post_tag') {
              if (!isset($tag_cloud['tags_page_caption']))
                $tag_cloud['tags_page_caption'] = __('(all tags)', 'categories-page');
  ...
  public function filter_plugin_actions($links) {
		...
		array_unshift($links, sprintf('<a href="%1$s">%2$s</a>', admin_url('widgets.php'), __('Settings', 'categories-page')));
		return $links;
	}
  ...
  private function admin_pointers() {
    ...
    if (get_option('tags_page_display_options_moved')) {
      $new_pointer_content = '<h3>' . __('Tags Page settings moved', 'categories-page') . '</h3>';
      $new_pointer_content .= '<p>' . __('Tags Page settings are moved directly into Tag&nbsp;Cloud widget options, please check Appearance&nbsp;&rarr;&nbsp;Widgets. There is also new option for link caption.', 'categories-page') . '</p>';
  ...
  public function parse_request(&$wp) {
    ...
    $count = $wpdb->get_var( $wpdb->prepare(
      "SELECT COUNT(*) FROM $wpdb->posts WHERE post_name=%s AND post_status='publish'",
      $tags_page_url
    ) );
    if ($count > 0)
  ...
  public function virtual_page_content($posts) {
    ...
    $p->post_content = "[get_categories]";
    $p->post_title = __('All Tags', 'categories-page');
    $p->post_excerpt = '';
    $p->post_status = 'publish';
    $p->ping_status = 'closed';
    $p->post_password = '';
    $p->post_name = __('all-categories', 'categories-page'); // slug
    $p->to_ping = '';
    $p->pinged = '';
  ...
  public function virtual_page_template() {
    get_template_part('page', 'categories-page');
    exit;
  }
}

$wp_CategoriesPage = new CategoriesPage();
?>